...
#============
# Generujemy proby licznosci n = 10 (100) z rozkladow: N(0, 1), Exp(1),
# LN(0, 1).
...
plot(ecdf(Y))
curve(pexp(x), col = 2, add = TRUE)

plot(ecdf(Z))
curve(plnorm(x), col = 2, add = TRUE)
...
lambda <- fexp$estimate
lambda
mu <- fln$estimate[[1]]
sigma <- fln$estimate[[2]]
mu
sigma
...
# Generujemy N = 10000 probek licznosci n = 100 z rozkladu F0=Exp(0.98)
# wybranego w Przykladzie 3
# i obliczamy odleglosc dystrybuant empirycznych od rozkladu F0 (wartosc
# statystyki Dn).
N <- 10000
n <- 100
...
par(mfrow = c(3, 1))
plot(danishuni$Loss)
hist(danishuni$Loss, prob = TRUE)
hist(danishuni$Loss[danishuni$Loss<30], prob = TRUE)
...
#-----------------------------------------------------
#1. Metoda MC wyznaczamy rozklad statystyki Dn. Generujemy N = 10000
# probek licznosci danych
# z rozkladu wybranego w punkcie A: F0 = LN(z
# wyestymowanymi parametrami) oraz obliczamy
# odleglosc dystrybuant
# empirycznych od wybranego rozkladu (czyli wartosc statystyki Dn).
N <- 10000
n <- length(Loss)
n
...
#2. Obliczamy dn, czyli wartosc statystyki Dn, dla danych Loss i rozkładu
# F0 wybranego w punkcie A.
dn_ln <-  ks.test(
  Loss,
  plnorm,
  fln$estimate[[1]],
...
# Odleglosc dystrybuanty empirycznej (wartosc statystyki Dn) dla Loss,
# oraz dystrybuanty F0
# jest istotnie większa od odleglosci obserwowanych
...
# zatem hipoteze o rownosci dystrybuant (F = F0, gdzie F poszukiwany
# rozklad) odrzucamy.

# C. Dodatkowo test zgodnosci, dla pozostalych dwoch odrzuconych w
# punkcie A rozkladow.
#------------------------------------------------------------------------------
# rozklad DN
N <- 10000
...
# odleglosc proby  od dystrybuant jest wieksza od maksymalnie obserwowanej
# z probek
...
# wartosci p-value rowne zero, oznaczają, że przy dowolnie przyjetym
# poziomie istotnosci
